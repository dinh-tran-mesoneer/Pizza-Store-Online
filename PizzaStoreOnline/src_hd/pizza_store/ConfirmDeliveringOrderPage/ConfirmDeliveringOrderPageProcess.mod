[Ivy]
[>Created: Mon Dec 12 11:43:53 ICT 2022]
...
' #txt
Cs0 f6 inActionCode 'int CONFIRM_DELIVERING_STATUS = 4;
param.order.status = CONFIRM_DELIVERING_STATUS;
ivy.persistence.JPA.merge(param.order);
ivy.log.info("Confirm delivering order with id: " + param.order.id);' #txt
...
Cs0 f7 guid 185049F715DD7D5D #txt
...
List<Order> deleveringOrders = ivy.persistence.JPA
	.createQuery("Select o FROM Order o WHERE o.status = :status")
	.setParameter("status", DELIVERING_STATUS)
	.getResultList();

out.listDeliveringOrders = deleveringOrders;

ivy.log.info("Load " + deleveringOrders.size() + " orders to delivering page");' #txt
